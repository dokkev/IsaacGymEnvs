...
class FrankaCubeSlide(PrivInfoVecTask):
    ...
    def store_proprio_hist(self):
        ...
        # get end effector pos and quat
        ee_states = torch.cat([self.states["eef_pos"], self.states["eef_quat"]], dim=1)  # [num_envs, 7]
        # get actions [num_envs, 8]
        actions = self.actions

        
        # combine cube states, eef states and actions
        all_states = torch.cat([cube_states, ee_states, actions], dim=1)  # [num_envs, 22]
        
        
    

        # proprio_hist_buf = [num_envs] x [prop_hist_len] x [prop_dim]
        all_states_dim = all_states.shape[1] # 7
        ...
        if all_states_dim > prop_his_buf_dim:
            raise ValueError(f"Proprioception buffer dimension mismatch! Cube state dim: {all_states_dim} > Proprioception buffer dim: {prop_his_buf_dim}")
        ...
        elif all_states_dim < prop_his_buf_dim:
            padding = torch.zeros((self.num_envs, (prop_his_buf_dim - all_states_dim)), device=self.device, dtype=torch.float)
            all_states = torch.cat([all_states, padding], dim=1) # shape: [num_envs, prop_his_buf_dim]
        ...
        self.proprio_hist_buf[:, -1, :] = all_states
    ...
    def _reset_init_cube_state(self, env_ids, check_valid=True):
        ...
        sampled_init_cube_state[:, :2] = init_cube_xy_state.unsqueeze(0) + \
